...
function ExperienceCard(props) {
  ...
  return (
    <Card
      className="text-white border flex-row"
    ...
    >
      {organizationLogo && (
        <img
          ...
          alt=""
          width={50}
          className="d-block align-self-start ms-4 mt-4"
        />
      )}
      <Card.Body className="me-4 ms-4">
        <p className="fw-bold mb-0 fs-5">{title}</p>
        <p className="fw-lighter mb-0">
          {organization} | <span className="fw-lighter">{experienceType}</span>
        </p>
        <p>{duration}</p>
        ...
        <p
          className={`text-wrap ${skills.length === 0 ? "d-none" : ""} skills`}
        >
